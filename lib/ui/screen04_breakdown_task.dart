        ...
        return AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () {
                setState(() {
                  myTask[widget.selectedIndex].breakdownItems.removeAt(index);
                });
                Navigator.pop(context);
              },
              child: const Text('Remove', style: TextStyle(color: Colors.red)),
  ...
  void _navigateToBreakdownNotes({int? editIndex}) async {
    final result = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => BreakdownNotesPage(
          swoNumber: widget.subtaskNumber,
          existingNote: editIndex != null ? myTask[widget.selectedIndex].breakdownItems[editIndex] : null,
        ),
      ),
    );
    ...
    if (result != null && result is Map<String, String>) {
      setState(() {
        if (editIndex != null) {
          myTask[widget.selectedIndex].breakdownItems[editIndex] = result;
          _showToast('Breakdown note updated', isError: false);

          widget.saveTasks(myTask);
        } else {
          myTask[widget.selectedIndex].breakdownItems.add(result);
          _showToast('Breakdown note added', isError: false);
    ...
    return _buildModernCard(
      child: Column(
        ...
        children: [
          ...
          // List of breakdown notes
          if (myTask[widget.selectedIndex].breakdownItems.isNotEmpty) ...[
            const SizedBox(height: 12),
            ...List.generate(myTask[widget.selectedIndex].breakdownItems.length, (index) {
              final note = myTask[widget.selectedIndex].breakdownItems[index];
              return Container(
                margin: const EdgeInsets.only(bottom: 8),
                padding: const EdgeInsets.all(14),
                decoration: BoxDecoration(
